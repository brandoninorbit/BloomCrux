...
import type { Flashcard, StandardMCQCard, CompareContrastCard, DragAndDropSortingCard, FillInTheBlankCard, ShortAnswerCard, TwoTierMCQCard, CERCard, SequencingCard, Topic, Deck, StudyMode, DeckProgress, UserPowerUps, PowerUpType, PurchaseCounts, CardFormat, UserXpStats, UserSettings, GlobalProgress, BloomLevel } from '@/stitch/types';
...
type QuestSession = {
  ...
  currentIndex: number;
  progressByLevel: Record<string, string[]>;
  completedLevels: BloomLevel[];
  totalCards: number;
  deckTitle?: string;
};

function isQuestSession(s: any): s is QuestSession {
  return (
    s &&
    typeof (s as any)?.currentIndex === "number" &&
    Array.isArray((s as any)?.levels) &&
    (s as any)?.progressByLevel &&
    Array.isArray((s as any)?.completedLevels)
  );
}
...
export default function QuestPage() {
  ...
  const [session, setSession] = useState<QuestSession | {id: string} | null>(null);
  ...
  const fetchSession = useCallback(async () => {
    ...
    try {
      ...
      const updatedSessionData = await getOrCreateQuestSession({
          uid: user.uid,
          deckId,
          fetchCardsByLevel: (lvl) => getCardsForDeckByBloomLevel(user.uid, deckId, lvl)
      });

      if (!isQuestSession(updatedSessionData)) {
          setSession(updatedSessionData);
          setLoading(false);
          return;
      }
      const updatedSession = updatedSessionData;
  ...
  const handleNextCard = useCallback(async () => {
    ...
    if (!isQuestSession(s) || (s.completedLevels.length === s.levels.length && s.levels.length > 0)) {
        setSession(s);
        setCurrentCard(null);
        setChecking(false);
        return;
    ...
    const order = updatedSessionData.progressByLevel[updatedSessionData.currentLevel] as string[];
    const cardId = order[updatedSessionData.currentIndex];
    const card = cardId ? await getCardById(user.uid, deckId, cardId) : null;
  ...
  if (!isQuestSession(session) || !currentCard) {
     return (
        <MissionComplete
            modeName="Quest"
            deckName={isQuestSession(session) ? session?.deckTitle || "Deck" : "Deck"}
            xp={150} // Placeholder
            coins={75} // Placeholder
            accuracy={92} // Placeholder
            questionsAnswered={isQuestSession(session) ? session?.totalCards || 0 : 0}
