...
function isRemixSession(s: any): s is RemixSession {
  return s && Array.isArray((s as any)?.order) && typeof (s as any)?.currentIndex === "number";
}
...
export default function RandomRemixPage() {
  ...
  const [session, setSession] = useState<RemixSession | { id: string } | null>(null);
  ...
  if (!isRemixSession(session) || !currentCard) {
    return (
       <MissionComplete
            ...
            questionsAnswered={isRemixSession(session) ? session?.totalCards || 0 : 0}
