...
import { getUserDecks } from "@/adapters/decks"; // Ã¢Â¬â€¦ keep decks from adapter
...
// Ã°Å¸â€Â¥ Firestore live folders


import { collection, onSnapshot, query, orderBy } from "@/lib/firebase.client";


import { getDb, getDbStrict, docDb, collectionDb } from "@/lib/firebase.client";
...
export default function DecksClient() {
  ...
  // Ã°Å¸â€Â Load decks (adapter) and mock when logged out
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const d = await getUserDecks(uid);
      ...
      setDecks((decksWithMockProgress) as unknown as DeckWithProgress[]);
  ...
  // Ã°Å¸â€Â´ LIVE Firestore subscription for folders (when logged in)
  ...
  useEffect(() => {
    ...
    console.log("[folders:read] subscribe", uid);
      ...
      (snap) => {
        ...
        const next: FolderSummary[] = snap.docs.map((d) => {
          ...
          return {
            ...
            // If you want a Date here and youÃ¢â‚¬â„¢re storing serverTimestamp:
