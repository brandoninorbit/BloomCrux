'use client';
import { collection, getDocs, query, where, addDoc, serverTimestamp, Timestamp, doc, setDoc, getDoc, runTransaction, writeBatch, increment, deleteDoc, onSnapshot, Unsubscribe, collectionGroup, orderBy, limit , getDbStrict } from '@/lib/firebase.client';
...
import { type User, updateProfile , getDbStrict } from '@/lib/firebase.client';
import { avatarFrames } from '@/config/avatarFrames';
import { ref as storageRef, uploadBytes, getDownloadURL , getDbStrict } from '@/lib/firebase.client';
...
export async function logCardAttempt(
  ...
  try {
    ...
    if (unlockedFrameName) {
      toast({
          title: `âœ¨ Avatar Frame Unlocked!`,
